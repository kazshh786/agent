...
function buildBookingWidgetSnippet({ analyticsKey, bookingApiEndpoint, paymentMode }) {
  ...
  return `<script data-ks-booking-widget>(function(){'use strict';var c=${config},root=document.querySelector('[data-ks-booking-root]'),state={catalog:null,service:null,slot:null,booking:null};if(!root)return;
...
function track(name,payload){if(window.KSAnalytics&&window.KSAnalytics.track)window.KSAnalytics.track(name,payload||{});}
...
async function paymentView(result){root.replaceChildren();var box=el('div');box.style.cssText='display:grid;gap:16px';box.appendChild(el('h2','Secure payment'));box.appendChild(el('p','Amount due: '+result.payment.currency+' '+(result.payment.amount/100).toFixed(2)));var card=el('div');card.style.cssText='padding:14px;border:1px solid #cbd5e1;border-radius:8px';box.appendChild(card);var button=el('button','Pay and confirm',{type:'button'});button.style.cssText='padding:13px 18px;border:0;border-radius:8px;background:#111827;color:white;font-weight:700;cursor:pointer';box.appendChild(button);box.appendChild(el('p','',{ 'data-status':''}));root.appendChild(box);var stripe=await loadStripe(result.payment.publishableKey),elements=stripe.elements(),cardElement=elements.create('card');cardElement.mount(card);button.addEventListener('click',async function(){button.disabled=true;status('Processing secure payment…');track('payment_started');var outcome=await stripe.confirmCardPayment(result.payment.clientSecret,{payment_method:{card:cardElement}});if(outcome.error){status(outcome.error.message||'Payment failed',true);button.disabled=false;return;}track('payment_completed',{paymentMode:c.paymentMode});for(var i=0;i<8;i++){await new Promise(function(r){setTimeout(r,750);});var check=await api('status',{}, {reference:result.booking.reference});if(check.booking.status==='CONFIRMED'){track('booking_confirmed',{paymentMode:c.paymentMode});return confirmView(check.booking);}}status('Payment received. Confirmation is still processing; keep this reference: '+result.booking.reference);});}
function bookingForm(){var selectedService=state.catalog.services.find(function(item){return item.id===state.service;});root.replaceChildren();var form=el('form');form.style.cssText='display:grid;gap:14px';form.appendChild(el('h2','Your details'));form.appendChild(el('p',selectedService.name+' · '+new Date(state.slot.start).toLocaleString()));form.appendChild(field('Full name','text','name'));form.appendChild(field('Email','email','email'));form.appendChild(field('Phone','tel','phone'));if(c.paymentMode==='customer_choice'){var choice=el('label');var check=el('input',null,{type:'checkbox',name:'payNow'});choice.appendChild(check);choice.appendChild(document.createTextNode(' Pay securely online now'));form.appendChild(choice);}else if(c.paymentMode==='deposit'||c.paymentMode==='full_payment'){form.appendChild(el('p',c.paymentMode==='deposit'?'A deposit is required to confirm this booking.':'Full payment is required to confirm this booking.'));}else form.appendChild(el('p','No online payment is required.'));var submit=el('button','Confirm booking',{type:'submit'});submit.style.cssText='padding:13px 18px;border:0;border-radius:8px;background:#111827;color:white;font-weight:700;cursor:pointer';form.appendChild(submit);form.appendChild(el('p','',{ 'data-status':''}));form.addEventListener('submit',async function(event){event.preventDefault();submit.disabled=true;status('Reserving your appointment…');track('customer_details_submitted');var data=new FormData(form);try{var result=await api('create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({serviceId:state.service,staffId:state.slot.staffId,startTime:state.slot.start,client:{name:data.get('name'),email:data.get('email'),phone:data.get('phone')},payNow:c.paymentMode==='customer_choice'?Boolean(data.get('payNow')):c.paymentMode==='deposit'||c.paymentMode==='full_payment',idempotencyKey:crypto.randomUUID()})});state.booking=result.booking;if(result.payment.required)return paymentView(result);track('booking_confirmed_no_payment',{paymentMode:c.paymentMode});confirmView(result.booking);}catch(error){status(error.message,true);submit.disabled=false;}});root.appendChild(form);}
