...
module.exports=async function(req,res){
  ...
  if(action==='availability'&&req.method==='GET'){
    const {serviceId,date,staffId='any'}=req.query;
    if(!UUID_RE.test(serviceId||'')||!/^\d{4}-\d{2}-\d{2}$/.test(date||'')||(staffId!=='any'&&!UUID_RE.test(staffId||'')))return errorResponse(res,400,'INVALID_REQUEST','Invalid availability request');
  ...
  if(action==='create'&&req.method==='POST'){
    const allowed=['serviceId','staffId','startTime','client','payNow','idempotencyKey'];
    ...
    if(!UUID_RE.test(req.body.serviceId||'')||!UUID_RE.test(req.body.staffId||'')||!UUID_RE.test(req.body.idempotencyKey||'')||!client||typeof client!=='object'||Array.isArray(client))return errorResponse(res,400,'INVALID_REQUEST','Invalid booking details');
    ...
    const result=await callKsOs(context,`/api/v1/service/tenants/${encodeURIComponent(context.tenantId)}/bookings`,{method:'POST',body:JSON.stringify({serviceId:req.body.serviceId,staffId:req.body.staffId,startTime:req.body.startTime,client:{name,email:email.toLowerCase(),phone},paymentMode:context.site.payment_mode,payNow:req.body.payNow===true,idempotencyKey:req.body.idempotencyKey})});
