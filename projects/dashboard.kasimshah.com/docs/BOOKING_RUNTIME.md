...
Required KS OS variables are `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `KS_OS_SERVICE_TOKEN`, `STRIPE_SECRET_KEY`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`, `STRIPE_WEBHOOK_SECRET` and a random `BOOKING_RATE_LIMIT_SALT` of at least 32 characters. Configure Stripe to deliver PaymentIntent events to `/api/v1/webhooks/stripe`.
